...
async def amain(text) -> None:
    """Main function"""
    file_path = r"TTS/data.mp3"

    if os.path.exists(file_path):
        os.remove(file_path)
    communicate = edge_tts.Communicate(text, 'en-US-ChristopherNeural',pitch='+5Hz', rate='+20%')
    await communicate.save(r'TTS/data.mp3')
      ...
      def Speak(*args,fn=fnsample,**kwargs):
            ...
            while 1:
                  try:
                        ...
                        pygame.mixer.init()
                        pygame.mixer.music.load(r"TTS/data.mp3")
